...
DROP TABLE IF EXISTS `bytemall_access_token`;
CREATE TABLE `bytemall_access_token`
(
    `id`             int(11) NOT NULL AUTO_INCREMENT,
    `key`            varchar(200) NOT NULL COMMENT 'access_token的key',
    `value`          varchar(300) NOT NULL COMMENT 'access_token的value',
    `expire_at`      bigint(13) NOT NULL COMMENT '过期时间戳',
    `expire_at_time` datetime(3) NOT NULL COMMENT '过期时间',
    `add_time`       datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`    datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `deleted`        tinyint(1) DEFAULT '0' COMMENT '逻辑删除',
    PRIMARY KEY (`id`),
    UNIQUE KEY `key` (`key`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='access_token存储表';