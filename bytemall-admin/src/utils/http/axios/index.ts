...
const transform: AxiosTransform = {
  ...
  transformRequestHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    const { errCode, errMsg, data: result } = data;

    // 这里逻辑可以根据项目进行修改
    const hasSuccess = data && Reflect.has(data, 'errCode') && errCode === ResultEnum.SUCCESS;
    ...
    if (errCode !== ResultEnum.SUCCESS) {
      ...
      if (errMsg) {
        createMessage.error(data.errMsg);
        Promise.reject(new Error(errMsg));
      } else {
        const msg = '操作失败,系统异常!';
  ...
  requestInterceptors: (config) => {
    ...
    if (token) {
      // jwt token
      config.headers['Bytemall-Token'] = token;
    }
    return config;
  },
